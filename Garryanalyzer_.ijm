...
//print("xc:"+xc+", yc:"+xc);  // Can be used to output xc and yc coordinates
...
roiManager("select", 1);
roiManager("rename", "2 Best-Fit Ellipse");
...
//saveAs("JPEG", ImageName+"_Highlight");
...
// Variables - This can be used to look at the equations with variables in context
//print("EllipticalScore = ("+bestResidualmean+"/"+majorAxis+")");
//print("PetioleBladeLengthRatio = ("+PetioleLength+"/"+BladeLength+")");

EllipticalScore = (bestResidualmean/majorAxis);
PetioleBladeLengthRatio = (PetioleLength/BladeLength);
...
// Information used under subsection O d - Appending variables to results
...
// O d - Append additional variables to Results, which is then exported in Data Export step
run("Set Measurements...", "area centroid perimeter fit shape feret's redirect=None decimal=4");
setResult("FileName", 0, ImageNameNoExtension);
setResult("EllipticalScore", 0, EllipticalScore);
setResult("PetioleBladeLengthRatio", 0, PetioleBladeLengthRatio);
setResult("SpeciesScore", 0, Species);
if (SpeciesR == 1) {
setResult("Species", 0, "Garryana");
   } else {
setResult("Species", 0, "Robur");
}


// P - Data Export. Optional. Data can be exported from ImageJ for later analysis. The line below is set up for individual images, and would need to be combined with like categories with a tool like PowerQuery.
	saveAs("Results", ImageDirectory+ImageNameNoExtension+".csv");
	print("Results saved to "+ImageNameNoExtension+".csv");
